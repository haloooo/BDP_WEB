...
class testcase_HomeUI_BasicCheck_0002(base_case.BaseCase):
    ...
    def testcase_HomeUI_BasicCheck_0002(self):
        ...
        self.assertTrue(check_result.check_pictures('disc_icon.png'),
                        'Disc icon display unnormal')

        print ('Step2: Press [ENTER] key on disc icon')
        qw_model_action_util.send_key('ENTER', 2)
        ...
        qw_model_action_util.send_key('STOP', 2)

        print ('Step4: Insert a USB, USB icon display normal')
        qw_model_action_util.go_diff_icon('USB')
        self.assertTrue(check_result.check_pictures('USB_device.png'),
        ...
        self.assertTrue(check_result.check_pictures('USB_device_appearance.png'),
                        'Open USB list failed')
        qw_model_action_util.send_key('RETURN', 2)

        print ('Step7: Can open Media Server list')
        qw_model_action_util.send_key('RIGHT')
        qw_model_action_util.send_key('ENTER')
        self.assertTrue(check_result.check_pictures('Media_Server.png'),
                        'Can not open Media Server list')
        qw_model_action_util.send_key('HOME')

        print ('Step10: Press [ENTER] key on a bivl app icon')
        qw_model_action_util.go_diff_icon('bivl app')

        print ('Step11: Can open the bivl app')
        self.assertTrue(check_result.check_pictures('bivl_app.png'),
